  ...
  return (
    ...
    >
      ...
      >
        ...
        <Link href="/pricing" className="px-8 py-4 text-blue-600 font-semibold hover:underline text-lg">
          View Pricing →
        </Link>
      </motion.div>
      ...
      >
        <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 border border-blue-100 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-blue-50/50 rounded-xl p-4 border border-blue-100 flex flex-col gap-3">
                <div className="flex justify-between items-center mb-1">
                    <div className={`h-4 rounded w-1/3 ${i === 1 ? 'bg-blue-200' : 'bg-blue-100'}`} />
                    <div className="h-4 w-4 rounded-full bg-blue-100" />
                </div>
                
                {/* Task Cards */}
                {[1, 2].map((j) => (
                    <div key={j} className="bg-white p-3 rounded-lg shadow-sm border border-blue-50">
